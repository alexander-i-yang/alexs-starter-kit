...
namespace Haze {
	...
	public static class Triangulator {
		...
		public struct Triangle {
			...
			{
				a = new Vector2((float)vertex.x, (float)vertex.y);
				b = new Vector2((float)vertex1.x, (float)vertex1.y);
				c = new Vector2((float)vertex2.x, (float)vertex2.y);
			}
		...
		{
			float eps = 0.00001f;
			float area = t.a.x * (t.b.y - t.c.y) + t.b.x * (t.c.y - t.a.y) + t.c.x * (t.a.y - t.b.y);
			return Math.Abs(area) < eps;
		}
		
		public static Triangle[] DenseTriangulate(Vector2[] mesh, float maxTriangleArea)
        {
            var ret = Triangulator.Triangulate(mesh.ToList()).ToArray();
            ret = ret.Where(t => !Triangulator.IsDegenerate(t)).ToArray();
            ret = DenseTriangle(ret, maxTriangleArea);
            return ret;
        }

        private static Triangulator.Triangle[] DenseTriangle(Triangulator.Triangle[] triangulated, float maxTriangleArea)
        {
            var ret = new List<Triangulator.Triangle>();
            foreach (var tri in triangulated)
            {
                ret.AddRange(DenseTriangle(tri, maxTriangleArea));
            }
            return ret.ToArray();
        }

        public static IEnumerable<Triangulator.Triangle> DenseTriangle(Triangulator.Triangle t, float maxTriangleArea)
        {
            IEnumerable<Vertex> points = t.arr.Select(x => new Vertex(x.x, x.y));
            ...
            return poly.Triangulate(options, quality).Triangles.Select(t => t.ToTriangulatorTriangle());
        }
	}
}